...
const ToggleInput: FC<ToggleInputProps> = ({
  ...
  return (
    <Field className="flex items-center">
      <Switch
        checked={enabled}
        onChange={(checked) => onChange(checked)} // Pass the new state directly

        disabled={disableInput}
        className={`group relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent ${enabled ? "bg-indigo-600" : "bg-gray-200"
